...
class StudentRiskAnalyzer:
    ...
    def calculate_risk_score_rule_based(self, student_data):
        attendance_score = max(0, (60 - student_data.get('attendance_percentage', 0)) / 60)
        academic_score = max(0, (40 - student_data.get('average_grade', 0)) / 40)
        fee_score = 0 if student_data.get('fee_status', 'paid') == 'paid' else 1
        backlog_score = min(1, student_data.get('backlogs', 0) / 5)
        risk_score = (
    ...
    def predict_risk_level(self, student_data):
        ...
        try:
            ...
            # FIXED: Use reindex for a more robust way to align columns
...
# ... (The rest of the Python file is unchanged from the last correct version) ...
# (It includes all endpoints: log_activity, reset_db, train_model, index, students, upload, analysis, profile, all advanced features, etc.)
...
def process_csv_data(df):
    ...
    if rows_to_insert:
        ...
        sql_query = f"INSERT OR REPLACE INTO students ({db_cols_str}) VALUES ({placeholders})"
...
def forecast_risk(student_id):
    ...
    last_date = datetime.strptime(history[-1]['analysis_date'], '%Y-%m-%d').date()
